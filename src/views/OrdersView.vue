...
const orderFlow = {
  ...
  pagado: ['pagado', 'enviado', 'cancelado'],
...
const statusOptions = computed(() => selected.value ? orderFlow[selected.value.estado] || [selected.value.estado] : [])
...
async function saveStatus() { try { error.value = ''; await api.patch(`/pedidos/${selected.value.id}/`, { estado: nextStatus.value }); selected.value = null; await load() } catch (err) { error.value = err.message } }
onMounted(load)
</script>
